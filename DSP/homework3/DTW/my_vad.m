...
% x = x(x_start:x_finish); 
% 
% %%%%%上述过程按照最后0.1秒作为环境噪声重做一遍
% 
% Ini = 0.1;          %初始静默时间
% Ts = 0.01;          %窗的时长
% Tsh = 0.005;        %帧移时长
...
% end
% 
% % IMN = mean(Erg_Vector(length(Erg_Vector) - sil:length(Erg_Vector)));  %静默能量均值（噪声均值）
% % IMX = max(Erg_Vector);          %短时平均幅度的最大值
% % I1 = 0.03 * (IMX-IMN) + IMN;    %I1，I2为初始能量阈值
% % I2 = 4 * IMN;
% % ITL = 100*min(I1,I2);            %能量阈值下限,前面系数根据实际情况更改得到合适结果
...
% 
% 
% ITL = 0.90 * max(Erg_Vector(length(Erg_Vector) - sil:length(Erg_Vector)));
% ITU = 1.5* ITL;
% IZC = mean(ZCR_Vector(length(Erg_Vector) - sil:length(Erg_Vector)));  
% stdev = std(ZCR_Vector(length(Erg_Vector) - sil:length(Erg_Vector))); %静默阶段过零率标准差
% 
% IZCT = min(ZTh,IZC+2*stdev);    %过零率阈值
% indexi = zeros(1,lengthX);      
% indexj = indexi;               
